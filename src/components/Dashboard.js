...
export default function Dashboard({ onPageChange, currentUser }) {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      // Fetch data siswa - FIXED: gunakan nama kolom baru
      ...
      // Hitung stats - FIXED: gunakan nama kolom baru
      const totalSiswa = students?.length || 0;
      const totalKelas = [...new Set(students?.map((s) => s.class_id))].length;
      const siswaAktif = students?.filter((s) => s.is_active).length || 0;
      const siswaLaki = students?.filter((s) => s.gender === "L").length || 0;
                ...
                <div className="flex items-center space-x-3">
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-sm ${
                      student.gender === "L" ? "bg-blue-400" : "bg-pink-400"
                    }`}>
