...
export default function Attendance({ currentUser }) {
  ...
  // Get homeroom class from current user, default to 7F if not set
  const homeroomClass = currentUser?.homeroom_class_id || "7F";
  const teacherId = currentUser?.teacher_id;

  // DEBUG: Log untuk cek teacher_id
  console.log("DEBUG - Current User:", currentUser);
  console.log("DEBUG - Teacher ID:", teacherId);

  // Filters - akan di-update setelah fetch assignments
  ...
  const fetchTeacherAssignments = async () => {
    ...
    try {
      ...
      // Set default filters berdasarkan assignments pertama
      if (data && data.length > 0) {
        setFilters((prev) => ({
          ...prev,
          class: homeroomClass || classes[0] || "",
          mapel: subjects[0] || "",
        }));
      }
      ...
      showNotification(
        "error",
        "Error memuat data assignment: " + error.message
      );
    } finally {
  ...
  const fetchStudents = async () => {
    if (!filters.class) return; // Tunggu sampai class ter-set
  ...
  // Initial load - fetch assignments dulu
  useEffect(() => {
    fetchTeacherAssignments();
  }, [teacherId]);

  // Load students & attendance setelah filters ready
  useEffect(() => {
    if (!loadingAssignments && filters.class) {
      fetchStudents();
  ...
  const handleFilterChange = (e) => {
    ...
    if (name === "type") {
      setFilters((prev) => ({
        ...
        [name]: value,
        class:
          value === "walikelas" ? homeroomClass : availableClasses[0] || "",
      }));
    } else {
      setFilters((prev) => ({
        ...prev,
  ...
  const saveAttendance = async () => {
    ...
    try {
      const attendanceRecords = students.map((student) => ({
        ...
        class: filters.class,
        date: filters.date,
        status: attendanceData[student.id]?.status || "alpa",
        notes: attendanceData[student.id]?.notes || "",
        type: filters.type,
        mapel: filters.type === "mapel" ? filters.mapel : null,
        teacher_name: currentUser?.full_name || "Guru",
  ...
  const handleDirectExport = async () => {
    ...
    try {
      ...
      const classFilter =
        filters.type === "walikelas" ? homeroomClass : filters.class;
      ...
      const normalizeStatus = (status) => {
        ...
        return normalized === "alpa" ? "alpa" : normalized;
      };
  ...
  const getStatusCount = (status) => {
    return students.filter(
      (student) => attendanceData[student.id]?.status === status
    ).length;
  };
  ...
  const statusCardColors = {
    ...
    alpa: "bg-red-500 hover:bg-red-600",
  };
  ...
  const statusSelectedColors = {
    ...
    alpa: "ring-2 ring-red-500 bg-red-100 border-red-500 text-red-800",
  };
          ...
          <div>
            ...
            {filters.type === "walikelas" && (
              <p className="text-xs text-gray-500 mt-1">
                Otomatis {homeroomClass} untuk Wali Kelas
              </p>
            )}
          </div>
