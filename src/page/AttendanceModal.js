...
const AttendanceModal = ({
  ...
  const fetchRekapData = async () => {
    if (
      ...
      onShowToast?.("error", "Pilih kelas dan mata pelajaran terlebih dahulu!");
      return;
    }

    if (!students || students.length === 0) {
      onShowToast?.("error", "Data siswa tidak tersedia!");
      return;
    }

    const dateRange = getDateRangeFromPeriod();
    if (!dateRange) {
      onShowToast?.("error", "Pilih periode yang valid!");
      return;
    }
    ...
    try {
      ...
      // FIXED: Gunakan field yang benar dari tabel students
      students.forEach((student, index) => {
        const studentId = student.id || student.student_id;
        const studentName = student.full_name || student.name;
        const studentNis = student.nis || student.student_id;
      ...
      console.log("DEBUG - Student summary initialized:", studentSummary);
      ...
      console.log("DEBUG - Final rekap data:", finalData);
      ...
      if (finalData.length > 0) {
        ...
        onShowToast?.("warning", "⚠️ Tidak ada data untuk ditampilkan");
      }
    } catch (error) {
      console.error("❌ Error in fetchRekapData:", error);
      onShowToast?.("error", "❌ Gagal memuat data rekap: " + error.message);
      setRekapData([]);
    } finally {
